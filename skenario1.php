...
<body class="bg-[#f2f7ff]">
    ...
    <div class="p-4 sm:ml-80 mt-4">
        <h1 class="font-semibold text-2xl">Hasil Perhitungan SAW - Skenario 1</h1>
        ...
        ORDER BY a.id_alternatif";
                            ...
                            $max_preferensi = 0;
                            $max_alternatif = "";  // Menyimpan nama alternatif dengan nilai preferensi tertinggi
                            $rows = [];
                            while ($row = $result->fetch_object()) {
                                $nilai_preferensi = ($row->C1 * 0.3794) + ($row->C2 * 0.1024) + ($row->C3 * 0.2488) + ($row->C4 * 0.1604) + ($row->C5 * 0.0434) + ($row->C6 * 0.0655);
                                $rows[] = ['data' => $row, 'nilai_preferensi' => $nilai_preferensi];
                                if ($nilai_preferensi > $max_preferensi) {
                                    $max_preferensi = $nilai_preferensi;
                                    $max_alternatif = $row->nama;  // Menyimpan nama alternatif dengan nilai tertinggi
                                }
                            }
                    ...
                    <!-- Menampilkan alternatif dengan nilai preferensi tertinggi
                    <p class="my-5">Skenario 1: Menggunakan 6 kriteria, yaitu Fungsionalitas, Usability, Efficiency, Reliability, Portability, Maintainability</p>
                    <p>Berdasarkan hasil dari skenario 1, alternatif <?php echo $max_alternatif; ?> lebih direkomendasikan sebagai Sistem Pemilihan Software Project Management Terbaik</p> -->
                </div>
            </div>
        </div>
    </div>
